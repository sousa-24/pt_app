"""
Endpoints para criar, listar, atualizar e eliminar sessões de treino.
"""
...
def create_training_session(
    ...
    if session.session_type == schemas.TrainingSessionType.individual:
        if session.client_id is None:
            raise HTTPException(status_code=400, detail="Indica o aluno.")
    ...
    new_session = models.TrainingSession(
        ...
        max_students=(
            ...
            if session.session_type == schemas.TrainingSessionType.group
            else None
        ),
        notes=session.notes,
    )
...
def update_training_session(
    ...
    if data.date is not None:
        validate_session_date(data.date)
...
@router.get(
    "/group_training_sessions/available",
    response_model=list[schemas.TrainingSessionResponse],
)
...
@router.post(
    "/group_training_sessions/{session_id}/enroll",
    response_model=schemas.TrainingSessionResponse,
)
